...
export class InfrastructureStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // using whatever IAM identity it already runs under, same as its existing
    // Cognito/EventBridge SDK calls; that identity needs s3:PutObject/GetObject/
    // DeleteObject on this bucket granted separately from wherever apps/api itself
    // is deployed (this stack doesn't own that compute).
    const attachmentsBucket = new s3.Bucket(this, 'AttachmentsBucket', {
      ...
      lifecycleRules: [
        ...
        { id: 'AbortIncompleteMultipartUploads', abortIncompleteMultipartUploadAfter: cdk.Duration.days(7) },
    ...
    new cdk.CfnOutput(this, 'AttachmentsBucketName', {
      ...
      description: 'S3 bucket for incident attachments — set as Aws__Attachments__BucketName on apps/api',
    });
    ...
    attachmentsBucket.grantRead(malwareProtectionRole);
    malwareProtectionRole.addToPolicy(new iam.PolicyStatement({
      actions: ['s3:PutObjectTagging', 's3:GetObjectTagging', 's3:GetBucketLocation'],
      resources: [attachmentsBucket.bucketArn, attachmentsBucket.arnForObjects('*')],
    }));
    malwareProtectionRole.addToPolicy(new iam.PolicyStatement({
      actions: ['iam:CreateServiceLinkedRole'],
      resources: ['arn:aws:iam::*:role/aws-service-role/malware-protection-plan.guardduty.amazonaws.com/*'],
      conditions: { StringLike: { 'iam:AWSServiceName': 'malware-protection-plan.guardduty.amazonaws.com' } },
    }));
    ...
    new cdk.CfnResource(this, 'AttachmentsMalwareProtectionPlan', {
      ...
      properties: {
        ...
        ProtectedResource: { S3Bucket: { BucketName: attachmentsBucket.bucketName } },

    ...
    : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // ITenantOwned entities below get `WHERE OrganizationId = @currentOrgId` applied
        // automatically, closed by default until an authorization handler populates it.
        ...
        modelBuilder.Entity<AnalyticsEvent>(b =>
        {
            ...
            // Closes the one gap where a tenant-owned entity had no filter. The analytics
            // worker writes via WorkerDbContextFactory.CreateUnscoped (unaffected), so this
            // gives any future org-scoped read the same default protection as other entities.
            b.HasQueryFilter(e => e.OrganizationId == currentOrganization.OrganizationId);
        });
    }
}

...
source sets `reportBatchItemFailures: true`. This is enforced end-to-end by
...
- Every worker pays one extra hop of latency (EventBridge → SQS → Lambda,
  not EventBridge → Lambda) in exchange for that retry/DLQ story. That's an
  acceptable latency-for-durability tradeoff here, since nothing in this
  pipeline is synchronous from the alert-sender's point of view (the

...
// Resolves (and lazily provisions) the local User row for the caller's Cognito identity.
// Access tokens carry `sub`/`username` but not `email`; mirrors AuthController's fallback.
...
public class CurrentUserService(SentinelOpsDbContext db, IHttpContextAccessor httpContextAccessor) : ICurrentUserService
{
    ...
    public async Task<User> GetOrProvisionAsync(CancellationToken ct = default)
    {
        ...
        // User isn't ITenantOwned and has no query filter to begin with; IgnoreQueryFilters()
